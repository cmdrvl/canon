...
nine-stage plan, typed local input refs, output refs, grain states, blockers, next actions,
...
shipped as a bounded offline executor for the current nine-stage plan: home cells, tile
section, evidence materialization, compilation, propagation, solve, explanation,
separation, and next-evidence selection. It delegates those registered executors to the
shared `canon.project.run.v2` runner, hashes explicit local
...
- **E5 — Genericity gate.** Repeat E1–E4 in one non-NYC county using only generic
  ...
  **2026-09-08 parcel-backed successor, still not gate execution.** The Franklin
  ...
  Admission retains 494,043 rows. Over the `80d0ea39-a5aa-4c27-a8d7-f662a4507257`
  build's 151 geocoded property
...
with reason code `rho_band_not_admissible`, carried on a new `#[serde(default)]`
`admission_reason: Option<String>` field of `GeoEvidenceAdmission` (the second half of the
same additive change). Every existing fixture and test is unchanged because both defaults
are absent. Relabeling an observer band as `LogicalRelaxation` to get it
...
| T68 | D6 | unit | three `GeoRhoContract` values with `basis: EmpiricalCalibration` over one `StructureCountInWindow` observation converted by `to_rho_observation`: flag absent; flag `true` with empty `calibration_blake3`; flag `true` and complete | flag absent: disposition `DiagnosticOnly`, admission reason `rho_band_not_admissible`, `hard_constraints.is_empty()`; complete: exactly one `IntegerSumBand` whose id is the observation's `rho:` generated id and `residual_model_count` narrows versus the unflagged compile | flag `true` with empty `calibration_blake3` refuses `invalid_input` with `detail["field"] = "calibration_blake3"`; a contract relabeled `LogicalRelaxation` to bypass the flag is caught by the T27 scan on `observer.rs` for the literal `LogicalRelaxation` | C11, C25; TH10 | `rho_band_not_admissible` |
...
| C25 | D2 to D6 do not modify `src/geo/composition.rs` or `src/geo/evidence.rs`, with exactly one exception: PK-D6a's additive `admissible_hard_band` flag on `GeoRhoBasis::EmpiricalCalibration` plus the `admission_reason` carrier on `GeoEvidenceAdmission` (§18.4 frozen decision 2026-09-02, T68, T69), after which the D1 residual digests are re-recorded; new modules call `solve_composition` as a black box, and generic modules carry no fixture, demo, or county literal | all §19.3 modules | §19.3 dependency direction, G7 |
