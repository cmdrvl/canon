use canon::{
    RefusalCode,
    entity::{
        CANON_ENTITY_BLOCK_VERSION, CANON_ENTITY_BLOCK_VERSION_V1,
        edge::{
...
fn edge_budget_refusal_rejects_over_budget_candidate_artifact_before_scoring() {
    ...
    assert_eq!(refusal.detail["reason"], "evidence_record_budget_exceeded");
    assert_eq!(
    ...
    assert_eq!(
        refusal.detail["partial_evidence_artifact_written"],
        json!(false)
    );
    assert!(
        refusal
            .next_command
            .as_deref()
            .is_some_and(|command| command.contains("canon entity evidence"))
    );
}

#[test]
fn edge_budget_refusal_rejects_legacy_or_stale_candidate_artifact() {
    let mut wrong_version = valid_artifact();
    wrong_version.version = CANON_ENTITY_BLOCK_VERSION.to_string();
    let refusal =
        validate_edge_candidate_artifact_before_scoring(&wrong_version, &valid_expectation())
            .expect_err("legacy block artifact version refuses");
    assert_eq!(refusal.code, RefusalCode::EEntityArtifactContract);
    assert_eq!(refusal.detail["stage"], "evidence");
    ...
    assert_eq!(refusal.detail["actual_version"], CANON_ENTITY_BLOCK_VERSION);
    assert_eq!(
        refusal.detail["partial_evidence_artifact_written"],
        json!(false)
    );
