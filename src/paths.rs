...
mod tests {
    ...
    #[test]
    fn default_witness_path_is_self_contained_without_home() {
        assert_eq!(
            default_witness_path_from_env(env_map(&[])),
            PathBuf::from(".cmdrvl/state/witness/witness.jsonl")
        );
    }
    ...
    fn migrating_legacy_witness_copies_without_deleting_source() {
        ...
        let legacy = home.path().join(".epistemic").join("witness.jsonl");
        fs::create_dir_all(legacy.parent().unwrap()).unwrap();
        fs::write(&legacy, "{\"tool\":\"canon-legacy\"}\n").unwrap();
        let values = [("HOME", home.path().as_os_str().to_os_string())];

        ensure_witness_migrated_from_env(env_map(&values)).unwrap();

        let canonical = home.path().join(".cmdrvl/state/witness/witness.jsonl");
        assert_eq!(
        ...
        let notices =
            fs::read_to_string(home.path().join(".cmdrvl/notices/deprecated-paths.jsonl"))
    ...
    fn migration_is_skipped_for_explicit_witness_override() {
        let home = TempDir::new().unwrap();
        let override_path = home.path().join("operator-ledger.jsonl");
        let values = [
        ...
        assert!(!home.path().join(".cmdrvl").exists());
    }
